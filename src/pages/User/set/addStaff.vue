<template>
  <div class="staff-index">
    <x-header :left-options="{showBack: true,backText: ''}">添加新成员</x-header>
      <group>
        <x-input text-align="right" v-model="addFrom.userCode" required>
        ...
        <x-input text-align="right" title="邮箱" v-model="addFrom.email" ></x-input>
         <x-input text-align="right" title="登录密码" type="password" v-model="addFrom.loginPwd" required>
           <span slot="label" class="input-right-solt">
            <span class="input-right-solt-required">*</span>
            登录密码
          </span>
         </x-input>
          <x-input text-align="right" title="确认密码" type="password" v-model="addFrom.comfirmLoginPwd" required>
            <span slot="label" class="input-right-solt">
            <span class="input-right-solt-required">*</span>
            确认密码
          </span>
...
export default {
  ...
  methods: {
    ...
    verify () {
      if (!this.addFrom.userCode) {
        this.$vux.toast.text('请填写员工编号!');
        return
      } else if (!this.addFrom.userName) {
        ...
        return
      } else if (!this.addFrom.loginPwd) {
        this.$vux.toast.text('请填写登录密码!');
        return
      } else if (!this.addFrom.comfirmLoginPwd) {
        this.$vux.toast.text('请填写确认密码!');
        return
      } else if (this.addFrom.comfirmLoginPwd != this.addFrom.loginPwd) {
        this.$vux.toast.text('两次填写的密码不一致!');
        return 
      }
      this.save()
    },
